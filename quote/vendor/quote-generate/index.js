...
class QuoteGenerate {
  ...
  async generate (backgroundColorOne, backgroundColorTwo, message, width, height, scale, emojiBrand, telegram) {
    ...
    // TeleBox userbot: 8th arg is either Telegraf or a plain {emojiId: Buffer} map
    // for custom emoji without a bot token (see generate.js collectCustomEmojiBuffers).
    if (telegram !== undefined) this.telegram = telegram
    scale = scale || 2
    ...
    // TeleBox: quote.ts may pre-download media into mediaCanvas (userbot has no bot file_id)
    if (message.mediaCanvas) {
      mediaCanvas = message.mediaCanvas
      mediaType = message.mediaType || 'photo'
      maxMediaSize = message.mediaMaxSize || null
